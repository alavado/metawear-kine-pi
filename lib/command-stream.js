...
function serializeDeviceState(device, dest, cache) {
...
async function start(options, config, cache, cacheFile) {
  ...
  for(let d of config['devices']) {
    ...
    try {
        ...
        await serializeDeviceState(device, cacheFile, cache)
  ...
  for(let it of devices) {
    ...
    Object.keys(config['sensors']).forEach(s => {
      if (!(s in SensorConfig)) {
        ...
        let state = DataCapture.createState((handler) => MetaWear.mbl_mw_datasignal_subscribe(SensorConfig[s].signal(d.board), handler), s, options);
