  ...
  for(let it of devices) {
    ...
    for(let s of Object.keys(config['sensors'])) {
      if (!(s in SensorConfig)) {
        ...
        let state = await DataCapture.createState((handler) => MetaWear.mbl_mw_datasignal_subscribe(SensorConfig[s].signal(d.board, true), handler), s, options);
    ...
    if (sensors.length != 0) {
      if (createWindow !== undefined) {
        ...
        createWindow(current_states, config['fps'], d.address, it[1], sensors.map(s => `${s}=${SensorConfig[s].odrToMs(config["sensors"][s]['odr'])}`), config['resolution'], x, y)
