...
function SensorFusionSetting(header, type, writeValue) {
    return new SettingHz(header, (board, config) => {
...
function TimedSensorSetting(csvHeader, getSignal, writeValue, exists) {
    ...
    return new Setting(csvHeader, async (board, config) => {
        let timer = await new Promise((resolve, reject) => 
            MetaWear.mbl_mw_timer_create_indefinite(board, config['odr'] * 1000, 0, MetaWear.FnVoid_TimerP.toPointer(ptr => resolve(ptr)))
        );
...
module.exports = {
    ...
    "Gyroscope" : new SettingHz("x-axis (deg/s),y-axis (deg/s),z-axis (deg/s)", (board, config) => {
        ...
        MetaWear.mbl_mw_gyro_bmi160_set_range(board, MetaWear.GyroBmi160Range.enums[range].value);
        MetaWear.mbl_mw_gyro_bmi160_set_odr(board, MetaWear.GyroBmi160Odr.enums[closest([25.0, 50.0, 100.0, 200.0], config['odr'])].value);
