...
function writeCartesianFloatType(data, state) {
    let value = data.parseValue()
    if ('session' in state) {
        state['session'].addLinearAcc(data.epoch, value.x, value.y, value.z);
    }
...
function writeFloatType(data, state, name) {
    ...
    if ('session' in state) {
        state['session'].addData(name, [data.epoch, entry[0]]);
    }

    state['stream'].write(util.format("%d,%s,%s,%s%s", data.epoch, formatTime(data.epoch), formatElapsed(data.epoch, state), 
...
function SettingHz(header, configure, start, signal, writeValue, exists) {
    return new Setting(header, configure, start, signal, writeValue, exists, odr => 1000.0 / odr)
}

function SensorFusionSetting(header, type, writeValue) {
    return new SettingHz(header, (board, odr) => {
...
function TimedSensorSetting(csvHeader, getSignal, writeValue, exists) {
    ...
    return new Setting(csvHeader, async (board, odr) => {
        let timer = await new Promise((resolve, reject) => 
            MetaWear.mbl_mw_timer_create_indefinite(board, odr * 1000, 0, MetaWear.FnVoid_TimerP.toPointer(ptr => resolve(ptr)))
        );

        return new Promise((resolve, reject) => {
            MetaWear.mbl_mw_event_record_commands(timer);
            MetaWear.mbl_mw_datasignal_read(getSignal(board))
            MetaWear.mbl_mw_event_end_record(timer, MetaWear.FnVoid_EventP_Int.toPointer((ptr, status) => {
                timers[board] = timer
                resolve(null);
            }))
        })
    ...
    }, getSignal, writeValue, exists, odr => 1000 * odr)
}

function TemperatureSetting() {
    var thermistorChannels = {}

    return new TimedSensorSetting("temperature (C)", board => MetaWear.mbl_mw_multi_chnl_temp_get_temperature_data_signal(board, thermistorChannels[board]), 
...
function HumiditySetting() {
    return new TimedSensorSetting("relative humidity (%)", board => MetaWear.mbl_mw_humidity_bme280_get_percentage_data_signal(board), 
...
module.exports = {
    "Accelerometer": new SettingHz("x-axis (g),y-axis (g),z-axis (g)", (board, odr) => {
    ...
    "Magnetometer" : new SettingHz("x-axis (T),y-axis (T),z-axis (T)", (board, odr) => {
        ...
        let value = data.parseValue()
        if ('session' in state) {
            state['session'].addMagData(data.epoch, value.x, value.y, value.z);
        }
    ...
    "Quaternion" : new SensorFusionSetting("w (number),x (number),y (number), z (number)", MetaWear.SensorFusionData.QUATERION, (data, state) => {
        let value = data.parseValue()
        if ('session' in state) {
            state['session'].addData("Quaternion", [data.epoch, value.w, value.x, value.y, value.z]);
        }
    ...
    "Euler Angles" : new SensorFusionSetting("pitch (deg),roll (deg),yaw (deg), heading (deg)", MetaWear.SensorFusionData.EULER_ANGLE, (data, state) => {
        let value = data.parseValue()
        if ('session' in state) {
            state['session'].addEulerAnglesData(data.epoch, value.pitch, value.roll, value.yaw, value.heading);
        }

        let entry = [value.pitch, value.roll, value.yaw, value.heading]
        state['stream'].write(util.format("%d,%s,%s,%s,%s,%s,%s%s", data.epoch, formatTime(data.epoch), formatElapsed(data.epoch, state), 
            entry[0].toFixed(3), entry[1].toFixed(3), entry[2].toFixed(3), entry[3].toFixed(3), os.EOL))
        state['update-graph'](entry)
    }),
    "Linear Acceleration" : new SensorFusionSetting("x-axis (g),y-axis (g),z-axis (g)", MetaWear.SensorFusionData.LINEAR_ACC, writeCartesianFloatType),
    "Gravity" : new SensorFusionSetting("x-axis (g),y-axis (g),z-axis (g)", MetaWear.SensorFusionData.GRAVITY, writeCartesianFloatType),
    "Ambient Light" : new SettingHz("illuminance (lx)", (board, odr) => {
        MetaWear.mbl_mw_als_ltr329_set_gain(board, MetaWear.AlsLtr329Gain._1X);
        MetaWear.mbl_mw_als_ltr329_set_integration_time(board, MetaWear.AlsLtr329IntegrationTime._100ms);
        MetaWear.mbl_mw_als_ltr329_set_measurement_rate(board, MetaWear.AlsLtr329MeasurementRate.enums[closest([0.5, 1.0, 2.0, 5.0, 10.0], 1000.0 / odr)].value);
        ...
        let value = data.parseValue()
        let entry = [value / 1000.0]

        if ('session' in state) {
            state['session'].addData("Ambient Light", [data.epoch, entry[0]]);
        }
    ...
    "Pressure" : new SettingHz("pressure (Pa)", (board, odr) => {
        ...
        case MetaWear.Const.MODULE_BARO_TYPE_BMP280:
            MetaWear.mbl_mw_baro_bmp280_set_standby_time(board, MetaWear.BaroBmp280StandbyTime.enums[closest([83.33, 13.51, 7.33, 3.82, 1.96, 0.99, 0.50, 0.25], odr)].value);
            break;
        case MetaWear.Const.MODULE_BARO_TYPE_BME280:
            MetaWear.mbl_mw_baro_bme280_set_standby_time(board, MetaWear.BaroBmp280StandbyTime.enums[closest([83.33, 13.51, 7.33, 3.82, 1.96, 0.99, 46.51, 31.75], odr)].value);
            break;
        }
