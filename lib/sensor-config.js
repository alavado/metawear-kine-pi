...
function writeCartesianFloatType(data, state) {
    ...
    if ('metacloud' in state) {
        state['metacloud'].addLinearAcc(data.epoch, value.x, value.y, value.z);
    }
...
function writeFloatType(data, state, name) {
    ...
    if ('metacloud' in state) {
        state['metacloud'].addData(name, [data.epoch, entry[0]]);
    }
...
module.exports = {
    ...
    "Magnetometer" : new SettingHz("x-axis (T),y-axis (T),z-axis (T)", (board, odr) => {
        ...
        let value = data.parseValue()
        if ('metacloud' in state) {
            state['metacloud'].addMagData(data.epoch, value.x, value.y, value.z);
        }
    ...
    "Quaternion" : new SensorFusionSetting("w (number),x (number),y (number), z (number)", MetaWear.SensorFusionData.QUATERION, (data, state) => {
        let value = data.parseValue()
        if ('metacloud' in state) {
            state['metacloud'].addData("Quaternion", [data.epoch, value.w, value.x, value.y, value.z]);
        }
    ...
    "Euler Angles" : new SensorFusionSetting("pitch (deg),roll (deg),yaw (deg), heading (deg)", MetaWear.SensorFusionData.EULER_ANGLE, (data, state) => {
        let value = data.parseValue()
        if ('metacloud' in state) {
            state['metacloud'].addEulerAnglesData(data.epoch, value.pitch, value.roll, value.yaw, value.heading);
        }
    ...
    "Ambient Light" : new SettingHz("illuminance (lx)", (board, odr) => {
        ...
        if ('metacloud' in state) {
            state['metacloud'].addData("Ambient Light", [data.epoch, entry[0]]);
        }
