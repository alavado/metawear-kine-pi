...
function updateState(state, line) {
    state['stream'].write(line)
    state[`update-graph`](line)
}
...
module.exports = {
    "Accelerometer": new Setting("x-axis (g),y-axis (g),z-axis (g)", (board, odr) => {
        ...
        if ('session' in state) {
            state['session'].addAccData(data.epoch, value.x, value.y, value.z);
        }
        
        let line = util.format("%d,%s,%s,%s,%s,%s,%s", data.epoch, formatTime(data.epoch), formatElapsed(data.epoch, state), 
    ...
    "Gyroscope" : new Setting("x-axis (deg/s),y-axis (deg/s),z-axis (deg/s)", (board, odr) => {
        ...
        let value = data.parseValue()
        if ('session' in state) {
            state['session'].addGyroData(data.epoch, value.x, value.y, value.z);
        }
    ...
    "Quaternion" : new SensorFusionSetting("w (number),x (number),y (number), z (number)", MetaWear.SensorFusionData.QUATERION, (data, state) => {
        ...
        let line = util.format("%d,%s,%s,%s,%s,%s,%s,%s", data.epoch, formatTime(data.epoch), formatElapsed(data.epoch, state), 
    ...
    "Gravity" : new SensorFusionSetting("x-axis (g),y-axis (g),z-axis (g)", MetaWear.SensorFusionData.GRAVITY, (data, state) => {
        let value = data.parseValue()
        if ('session' in state) {
            state['session'].addGravity(data.epoch, value.x, value.y, value.z);
        }
